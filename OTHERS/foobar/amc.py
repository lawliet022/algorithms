...
def solution(m):
    ...
    for i in range(len(res)):
        lcm = lcm * res[i].denominator // gcd(lcm,res[i].denominator)
    ans = []
    for i in range(len(res)):
        ans.append([lcm // res[i].denominator * res[i].numerator, lcm])	
    ...
    for x in range(len(ans)):
        answer.append(int(ans[x][0]//res_gcd))
    answer.append(int(lcm//res_gcd))
